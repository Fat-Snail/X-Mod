    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (args.Length == 0)
            {
                ...
                Console.WriteLine("\t-mi <PATH>\t\tModel.xml输入目录，为空则是根目录");
                Console.WriteLine("\t-output <PATH>\t\t输出目录，为空则是根目录");
            ...
            {
                //只读两个参数mi，output
                var index = Array.IndexOf(args, "-mi");
                if (index > -1)
                {
                    file = args[index + 1].Trim();
                }

                index = Array.IndexOf(args, "-output");
                if (index > -1)
                {
                    output = args[index + 1].Trim();
                    XTrace.LogPath = output;
                }
        ...
        static void Build(String modelFile, String output)
        {
            ...
            if (!output.IsNullOrEmpty())
            {
                ...
                Console.WriteLine("保存目录：{0}", output);
            }
